...
window.createFinanceUI = function ({ getState, save, renderAll, money, esc, uid, today, switchView, toast, openExpense }) {
  ...
  let selectedPaycheck = state().paydays.some(row => row.id === state().ui.plannerPaycheck && row.date.startsWith(state().selectedMonth)) ? state().ui.plannerPaycheck : ''; let viewedMonth = selectedPaycheck ? state().selectedMonth : ''; let billMonth = ''; let editing = null; let pendingDeletion = null
  ...
  const paycheckFlow = p => `<section class="paycheck-flow" aria-label="Paycheck money flow"><div class="flow-intro"><span class="eyebrow">FOLLOW YOUR MONEY</span><h3>From payday to what is left</h3><p>Bills → Savings → Debt → Remaining. Adjust your plan below and see each balance update.</p></div><div class="pay-flow-head" aria-hidden="true"><span>Your plan</span><span>Amount</span><span>Left after this step</span></div><ol>${F.flow(p).filter(row => row.label !== 'Everyday spending' || row.amount !== 0).map((row, index) => `<li class="pay-flow-row"><span><i aria-hidden="true">${index+1}</i><span><strong>${row.label}</strong><small>${row.detail}</small></span></span><b class="money">${row.income || row.amount < 0 ? '+' : '−'}${money(Math.abs(row.amount))}</b><span class="pay-flow-left"><small>Left after ${row.income ? 'income' : row.label.toLowerCase()}</small><strong class="money">${money(row.remaining)}</strong></span></li>`).join('')}</ol><div class="pay-flow-result${p.remaining < 0 ? ' over-budget' : ''}" role="status"><span><strong>${p.remaining < 0 ? 'Overallocated' : 'Remaining to use or keep'}</strong><small>${p.remaining < 0 ? 'Reduce an allowance or review assigned bills.' : 'After bills, savings, debt and everyday spending.'}</small></span><b class="money">${money(p.remaining)}</b></div><p class="finance-hint">Planning amounts, not your bank balance. Recorded activity uses its allowance; it is not subtracted twice. A negative savings allocation means returned money is available again.</p></section>`
  const overview = p => `<section id="paycheckOverview" aria-label="Paycheck totals"><div class="paycheck-balance${p.remaining < 0 ? ' over-budget' : ''}" role="status"><span>${p.remaining < 0 ? 'Over budget' : 'Remaining'}<small>${esc(p.paycheck.name)} · ${esc(p.paycheck.date)}</small></span><strong class="money">${money(p.remaining)}</strong></div>${paycheckFlow(p)}<details class="visual-details"><summary>How this paycheck is calculated</summary><dl class="paycheck-totals"><div><dt>Income</dt><dd class="money">${money(p.paycheck.amount)}</dd></div><div><dt>Used / assigned</dt><dd class="money">${money(p.used)}</dd></div><div><dt>Still reserved</dt><dd class="money">${money(p.reserved)}</dd></div><div><dt>Total used + reserved</dt><dd class="money">${money(p.total)}</dd></div></dl><p class="finance-hint">Remaining = income − used/assigned − still reserved. Assigned bills count even when unpaid. This is a plan, not a bank balance.</p><div class="paycheck-breakdown"><div><span>Bills assigned</span><b class="money">${money(p.buckets.bills)}</b></div><div><span>Spending recorded</span><b class="money">${money(p.actual.expenses)}</b></div><div><span>Debt payments recorded</span><b class="money">${money(p.actual.debt)}</b></div><div><span>Savings contributed, less returns</span><b class="money">${money(p.actual.savings)}</b></div></div><p class="finance-hint">Not yet used: debt ${money(p.pending.debt)} · savings ${money(p.pending.savings)} · other ${money(p.pending.other)}. Recorded transactions consume these allowances; they are not counted twice.</p></details></section>`
  ...
  const render = () => {
    const s = state()
    $('#appShell').classList.remove('paycheck-only')
    ...
    if (selectedPaycheck && !s.paydays.some(row => row.id === selectedPaycheck)) selectedPaycheck = s.paydays.find(row => row.date.startsWith(viewedMonth))?.id || ''
    s.ui.plannerPaycheck = selectedPaycheck
  ...
  const bind = () => {
    ...
    document.addEventListener('change', event => {
      ...
      if (event.target.id === 'planPaycheck') { selectPaycheck(event.target.value, true); save(); renderAll() }
    ...
    $('#newPaycheckForm').addEventListener('submit', event => {
      ...
      state().paydays.push(row); state().selectedMonth = row.date.slice(0, 7); viewedMonth = state().selectedMonth; billMonth = viewedMonth; selectedPaycheck = row.id; state().ui.plannerPaycheck = row.id; state().selectedDate = row.date; state().ui.printMonth = state().selectedMonth
  ...
  const selectPaycheck = (id, navigate = false) => { const pay = state().paydays.find(row => row.id === id); if (pay) { selectedPaycheck = id; state().ui.plannerPaycheck = id; if (navigate) { state().selectedMonth = pay.date.slice(0, 7); state().selectedDate = pay.date; state().ui.printMonth = state().selectedMonth } viewedMonth = state().selectedMonth; billMonth = viewedMonth } else if (!id) { selectedPaycheck = ''; state().ui.plannerPaycheck = '' } }
