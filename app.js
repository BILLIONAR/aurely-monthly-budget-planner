(() => {
  ...
  const starterState = {
    ...
    quickPresets: typeof BudgetExtras !== 'undefined' ? clone(BudgetExtras.defaults) : [], wishlist: [], brainDump: [], notes: {}, monthNotes: {}, intention: '', dailyIntentions: {}, noSpendDays: [],
  ...
  const mergeState = (saved) => {
    ...
    if (typeof BudgetExtras !== 'undefined') {
      try { BudgetExtras.validatePresets(merged.quickPresets) } catch { merged.quickPresets = clone(BudgetExtras.defaults) }
      merged.goals.forEach(goal => { if (goal.targetDate && !BudgetExtras.validDate(goal.targetDate)) goal.targetDate = '' })
    }
  ...
  const setAnimatedMoney = (selector, value, suffix = '') => {
    ...
    const target = num(value); const previous = Number(element.dataset.moneyValue); element.dataset.moneyValue = String(target); element.dataset.moneySuffix = suffix
  ...
  const renderGoals = () => {
    $('#goalGrid').innerHTML = state.goals.length ? state.goals.map((goal) => {
      ...
      return `<article class="goal-card paper-card"><div class="goal-ring" style="--progress:${percent * 3.6}deg"><span>${Math.round(percent)}%</span></div><div class="goal-copy"><span class="eyebrow">SAVING GOAL</span><h2>${esc(goal.name)}</h2>${financeUI.balanceChart(goal, 'goal')}${financeUI.accountSummary(goal, 'goal')}${typeof BudgetExtras !== 'undefined' ? extras.goalMarkup(goal) : ''}<small>${left <= 0 ? 'Goal reached' : months ? `About ${months} month${months === 1 ? '' : 's'} at ${masked(goal.monthly)}/month` : `${masked(left)} left · no monthly plan yet`}</small><label class="check-row compact-check"><input type="checkbox" data-goal-keepout="${goal.id}"${goal.keepOut !== false ? ' checked' : ''}><span>Keep deposits out of safe-to-use</span></label>${financeUI.buttons(goal, 'goal')}${financeUI.history(goal, 'goal')}<div class="row-actions"><button type="button" data-goal-remove="${goal.id}">Remove goal</button></div></div></article>`
  ...
  const renderDebts = () => {
    ...
    $('#debtGrid').innerHTML = state.debts.length ? state.debts.map((debt) => { const progress = clamp((num(debt.startingBalance) - num(debt.balance)) / Math.max(1, num(debt.startingBalance)) * 100, 0, 100); return `<article class="paper-card debt-card"><div class="card-heading"><div><span class="eyebrow">${num(debt.apr).toFixed(2)}% APR</span><h2>${esc(debt.name)}</h2></div><strong class="money">${masked(debt.balance)}</strong></div>${financeUI.balanceChart(debt, 'debt')}${financeUI.accountSummary(debt, 'debt')}${typeof BudgetExtras !== 'undefined' ? extras.debtMarkup(debt) : ''}<div class="debt-track"><i style="width:${progress}%"></i></div><p>${Math.round(progress)}% paid down · usual payment ${masked(debt.payment)}</p>${financeUI.buttons(debt, 'debt')}${financeUI.history(debt, 'debt')}<div class="row-actions"><button type="button" data-debt-remove="${debt.id}">Remove account</button></div></article>` }).join('') : '<article class="paper-card empty-state"><strong>No debt tracked</strong><p>If this screen is not useful, Simple mode hides it.</p></article>'
  }
  ...
  const renderAll = () => {
    applyTheme(); renderWishlist(); renderOverview(); renderMonthRail(); renderSummary(); renderCheckins(); renderMonth(); renderToday(); renderBills(); renderIncome(); financeUI.render(); renderGoals(); renderDebts(); renderEnvelopes(); renderTheme(); renderHelp(); renderSettings(); $('#exampleBanner').hidden = !state.profile.sampleData || state.profile.sampleDismissed; $('#helpButton').classList.toggle('active', state.ui.helpMode); $('#helpButton').setAttribute('aria-pressed', String(state.ui.helpMode)); $('#focusButton').classList.toggle('active', state.ui.focusMode); $('#focusButton').setAttribute('aria-pressed', String(state.ui.focusMode)); if (typeof BudgetExtras !== 'undefined') extras.render()
  }
  ...
  const bindDelegatedEvents = () => document.addEventListener('click', (event) => {
    ...
    if (insight) { const selector=insight.dataset.insightHost; insightSelection[selector]=insight.dataset.insightGroup; renderInsights(selector,state.selectedMonth); if (typeof BudgetExtras !== 'undefined') extras.refreshCharts(); $(`${selector} [data-insight-group="${insight.dataset.insightGroup}"]`).focus() }
  ...
  const bindForms = () => {
    ...
    $('#goalForm').addEventListener('submit', (event) => { event.preventDefault(); const form = event.currentTarget; const data = Object.fromEntries(new FormData(form)); state.goals.push({ id: uid(), name: data.name.trim(), target: num(data.target), targetDate: data.targetDate || '', saved: Math.max(0, num(data.saved)), monthly: Math.max(0, num(data.monthly)), keepOut: true, contributions: [] }); state.profile.sampleData = false; form.reset(); form.elements.saved.value = 0; form.elements.monthly.value = 25; if (form.elements.targetDate) form.elements.targetDate.value = '';  save(); renderAll(); toast('Goal created') })
  ...
  const parseBackup = (text) => {
    ...
    if (typeof BudgetExtras !== 'undefined' && candidate.quickPresets !== undefined) BudgetExtras.validatePresets(candidate.quickPresets)
    if (candidate.goals.some(goal => goal.targetDate !== undefined && goal.targetDate !== '' && !validDate(goal.targetDate))) throw new Error('Invalid goal target date')
  ...
  const extras = typeof BudgetExtras !== 'undefined' ? BudgetExtras.create({ getState: () => state, save, renderAll, money: masked, formatMoney: money, esc, uid, today: () => currentDate, toast, insightData: insightSnapshot }) : null
  extras?.bind()
  financeUI.bind()
