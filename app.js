(() => {
  ...
  const starterState = {
    ...
    profile: { name: '', title: 'My money quest', checking: 0, setAside: 0, usualPaycheck: 0, payCycle: 'biweekly', payAnchor: '', weekendEarly: false, depth: 'full', sampleData: true, sampleDismissed: false },
    ...
    milestones: { reached: {} }, game: { xp: 120, awarded: [] }, theme: clone(baseTheme)
  }
  const blankState = () => ({
    ...
    profile: { ...clone(starterState.profile), sampleData: false, sampleDismissed: true, title: 'My money plan' }, milestones: { reached: {} }, game: { xp: 0, awarded: [] }
  })
  const mergeState = (saved) => {
    ...
    const merged = {
      ...
      game: { xp: Math.max(0, num(saved.game?.xp ?? starterState.game.xp)), awarded: Array.isArray(saved.game?.awarded) ? saved.game.awarded : [] },
  ...
  const level = () => Math.floor(state.game.xp / XP_STEP) + 1
  const levelNames = ['Fresh Start', 'Budget Scout', 'Plan Keeper', 'Calm Strategist', 'Momentum Maker', 'Money Pathfinder']
  const questKey = (id) => `${id}:${currentDate}`
  const questDone = (id) => state.game.awarded.includes(questKey(id))
  const questData = () => [
    { id: 'spend', icon: '＋', title: 'Daily check-in', detail: 'Log a purchase or a no-spend day', points: 25, view: 'today' },
    { id: 'bill', icon: '✓', title: 'Clear a bill', detail: 'Mark one tracked bill paid', points: 30, view: 'bills' },
    { id: 'goal', icon: '◆', title: 'Power up a goal', detail: 'Add a little to any goal', points: 20, view: 'goals' }
  ]
  const celebrate = (big = false) => {
    if (state.theme.calmMode || matchMedia('(prefers-reduced-motion: reduce)').matches) return
    const layer = $('#celebrationLayer')
    const colors = ['#ffd174', state.theme.accent, state.theme.main, '#a999c7', '#70b9a0']
    layer.innerHTML = Array.from({ length: big ? 20 : 10 }, (_, i) => `<i style="--spark-left:${35 + Math.random() * 30}%;--spark-drift:${-90 + Math.random() * 180}px;--spark-delay:${Math.random() * .12}s;--spark-color:${colors[i % colors.length]}"></i>`).join('')
    requestAnimationFrame(() => layer.classList.add('burst'))
    setTimeout(() => { layer.classList.remove('burst'); layer.innerHTML = '' }, 1100)
  }
  const award = (id, points, label) => {
    if (questDone(id)) return false
    const oldLevel = level()
    state.game.awarded.push(questKey(id)); state.game.xp += points; save(); renderGame(); celebrate(level() > oldLevel)
    toast(level() > oldLevel ? `Level up! ${levelNames[Math.min(level() - 1, levelNames.length - 1)]} · +${points} XP` : `${label} · +${points} XP`)
    return true
  }
  ...
  const renderGame = () => {
    const progress = state.game.xp % XP_STEP; const quests = questData(); const done = quests.filter((item) => questDone(item.id)).length
    $('#levelBadge').textContent = level(); $('#levelName').textContent = levelNames[Math.min(level() - 1, levelNames.length - 1)]; $('#xpText').textContent = `${progress} / ${XP_STEP} XP`; $('#xpBar').style.width = `${progress}%`; $('#questProgress').textContent = `${done} / 3`; $('#todayQuestScore').textContent = `${done} / 3`
    $('#dailyQuestList').innerHTML = quests.map((item) => `<button class="quest-item${questDone(item.id) ? ' complete' : ''}" type="button" data-quest-view="${item.view}"><span class="quest-icon">${questDone(item.id) ? '✓' : item.icon}</span><span><strong>${item.title}</strong><small>${questDone(item.id) ? 'Quest complete for today' : item.detail}</small></span><b>${questDone(item.id) ? 'DONE' : `+${item.points} XP`}</b></button>`).join('')
    $('#todayQuestList').innerHTML = quests.map((item) => `<button class="mini-quest${questDone(item.id) ? ' complete' : ''}" type="button" data-quest-view="${item.view}"><i>${questDone(item.id) ? '✓' : item.icon}</i><span><strong>${item.title}</strong><small>${questDone(item.id) ? 'Cleared today' : item.detail}</small></span><b>${questDone(item.id) ? 'DONE' : `+${item.points}`}</b></button>`).join('')
  }
  ...
  const bindForms = () => {
    $('#expenseForm').addEventListener('submit', (event) => {
      ...
      state.profile.sampleData = false; state.selectedMonth = item.date.slice(0, 7); state.selectedDate = item.date; state.noSpendDays = state.noSpendDays.filter((day) => day !== item.date); financeUI.selectPaycheck(item.paycheckId); quickDialog.close(); save(); renderAll(); if (!data.id && item.date === currentDate) award('spend', 25, 'Spend captured'); toast(data.id ? 'Spending updated' : `${money(item.amount)} saved`)
    })
  ...
  const financeUI = createFinanceUI({ getState: () => state, save, renderAll, money: masked, esc, uid, today: () => currentDate, switchView, toast, award, openExpense })
  financeUI.bind()
